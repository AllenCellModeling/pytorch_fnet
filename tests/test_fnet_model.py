...
def test_train_on_batch():
    ...
    cost_exp = (
        model.train_on_batch(
            x_batch[1:-1], y_batch[1:-1], weight_map_batch[1:-1]
        )
        * (batch_size - 2) / batch_size  # account for change in batch size
    )
...
def test_test_on_batch():
    ...
    loss_weight_zero = model.test_on_batch(
        x_batch, y_batch, torch.zeros(shape_batch)
    )
