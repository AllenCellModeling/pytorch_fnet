...
class Net(torch.nn.Module):
    def __init__(self, out_channels = 1, dropout_rate = 0.2):
        super().__init__()
        mult_chan = 32
        depth = 4
        ...
        self.net_recurse = _Net_recurse(n_in_channels=1, mult_chan=mult_chan, depth=depth, dropout_rate = dropout_rate)
...
class _Net_recurse(torch.nn.Module):
    def __init__(self, n_in_channels, mult_chan=2, depth=0, dropout_rate = 0.2):
        ...
        self.sub_2conv_more = SubNet2Conv(n_in_channels, n_out_channels, dropout_rate = dropout_rate)
        ...
        if depth > 0:
            ...
            self.dr0 = torch.nn.Dropout3d(dropout_rate, inplace=True)
            ...
            # self.dr1 = torch.nn.Dropout3d(0.2, inplace=True)
...
class SubNet2Conv(torch.nn.Module):
    def __init__(self, n_in, n_out, dropout_rate = 0.2):
        super().__init__()
        
        self.net = torch.nn.Sequential(
                    torch.nn.Conv3d(n_in,  n_out, kernel_size=3, padding=1),
                    torch.nn.Dropout3d(dropout_rate, inplace=True),
                    torch.nn.BatchNorm3d(n_out),
                    torch.nn.ReLU(),
                    torch.nn.Conv3d(n_out, n_out, kernel_size=3, padding=1),
                    torch.nn.Dropout3d(dropout_rate, inplace=True),
                    torch.nn.BatchNorm3d(n_out),
                    torch.nn.ReLU())
