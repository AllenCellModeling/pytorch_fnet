...
GPU_IDS=${6:-0}
PATCH_SIZE_MULT=${7:-1}
RUN_DIR=saved_models/${DATASET}/fs_${N_SOURCE_POINTS}/lag_${N_OFFSET}/excl_${EXCLUDE_SCENES}
PATH_DATASET_CSV=data/csvs/train_movie_${DATASET}.csv
CLASS_DATASET=CziMovieDataset
NN_MODULE=fnet_nn_3d_fs
...
python train_iterative_model.py \
       ...
       --fds_kwargs '{"n_source_points": 10, "n_offset": '${N_OFFSET}', "exclude_scenes": "'${EXCLUDE_SCENES}'"}' \
       --nn_kwargs '{"in_channels": 10, "select_channels": '${N_SOURCE_POINTS}'}' \
