...
def main():
    ...
    parser.add_argument('--path_source_dir', help='path to directory containing results.csv and evaluation images')
    parser.add_argument('--train_split', type=int, default=34, help='number of images in training set')
    opts = parser.parse_args()

    tag = os.path.basename(opts.path_source_dir)
    path_source_csv = os.path.join(opts.path_source_dir, 'results.csv')
    path_train_csv = os.path.join(opts.path_save_dir, '{:s}_train.csv'.format(tag))
    path_test_csv = os.path.join(opts.path_save_dir, '{:s}_test.csv'.format(tag))
    df_source = pd.read_csv(path_source_csv)
    print('read csv:', path_source_csv)
    df_cleaned = clean_df(df_source)
    df_train, df_test = shuffle_split_df(df_cleaned, opts.train_split)
