...
def main():
    ...
    for path_model_dir in opts.path_model_dir:
        ...
        path_model = os.path.join(path_model_dir, 'model_{:s}.p'.format('' if not opts.iter_mode else '_'.join([str(i) for i in source_points])))
    ...
    dataset.source_points = source_points
    ...
    for idx in indices:
        ...
        chan_mult_factor = 2 if opts.nn_module == 'fnet_nn_3d_uncertainty' else 1
        ...
        for b in range(n_batches):
            ...
            for name_model in models:
                ...
                n_pred = 50
                prediction = model.predict(sig_b).numpy()[:,:,:,0:112,0:176]
                pred_mean = prediction[:,0]
                pred_var = np.exp(prediction[:,1])
                pred_mean_sq = pred_mean**2
                for x in range(n_pred-1):
                    #janky because of weird pytorch issues
                    prediction_next = model.predict(sig_b).numpy()[:,:,:,0:112,0:176]# if model is not None else None
                    pred_mean += prediction_next[:,0]
                    pred_var += np.exp(prediction_next[:,1])
                    pred_mean_sq += prediction_next[:,0]**2
                #compute sample mean of predicted mean channels
                pred_mean /= n_pred
                #combine estimate of aleatoric variance and epistemic variance
                pred_var /= n_pred
                pred_var = pred_var + pred_mean_sq/n_pred - pred_mean**2
                save_tiff_and_log('prediction', pred_mean, path_tiff_dir, entry, opts.path_save_dir, lag + 1)
                save_tiff_and_log('prediction_sd', pred_var**0.5, path_tiff_dir, entry, opts.path_save_dir, lag + 1)
                entry['mse_lag1'] = np.mean((pred_mean - targetArr[b, 0])**2)
                if not opts.no_prediction_unpropped:
                    ar_pred_unpropped = propper.undo_last(prediction[0, 0, ])
